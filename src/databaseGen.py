...
def imageCreating(img, alpha, nMean, nStd, shape):
    ...
    rebin = 1
    xx, yy = np.meshgrid(range(int(1024 - shape[0] / 2), int(1024 + (shape[0] / 2)), rebin),
                         range(int(1024 - shape[0] / 2), int(1024 + shape[0] / 2), rebin))
...
nRuns = 250000
imShape = (256, 256)
f = h5py.File('Run_' + str(date.today()) + '.h5', 'w')
